...
Introduction to Remote Sensing is the basic remote sensing course for Geography students at Humboldt-Universität zu Berlin. In this course, you will be exposed to the theoretical fundaments and introductory applications of remote sensing.

We link the course contents closely to ongoing research of the department´s remote sensing group - the [Earth Observation Lab](https://hu.berlin/eo-lab). During the first weeks, we will focus on datasets acquired for the region of Berlin and Brandenburg. 

Course topics contain 

- visual image interpretation
- spectrometry
- acquisition and processing of optical satellite data
- vegetation analyses
- principles of image classification
- fundaments of accuracy assessments
- time series analyses
- change detection

After completing this course, you are able to acquire and process remotely sensed datasets using basic analytical tools and generate insights that help you to answer research questions in other projects. 

The course is based on open source software.

------

## Requirements 

*Active participation* is expected. Successful completion of the module requires you to ask questions and get involved in discussion rounds. 

*Exercises* will be worked on in teams of four and submitted individually on a weekly basis. We grade submissions based on a point scheme, where each submission can earn you at most three points: 
...
*A brief presentation* of exercise results towards the end of the class is mandatory for each participant.
...
The video lectures for this course are provided via moodle on a weekly basis. The videos will be made available one week ahead of time to foster asynchronous learning. While watching the lectures, please make sure you understand the contents, and revisit passages which you did not understand. 

We start the weekly meetings with a Q&A and discussion of the lecture contents, so please take notes on open questions of understanding or discussion.

## The seminar

Seminar contents are of an applied nature and closely related to the lecture contents. This website contains the neccessary seminar materials. For every week, we provide a background section, introducing the topic session, as well as the instructions and datasets needed to complete the weekly exercise.

Completed exercises must be submitted individually (by every student) via [moodle](http://moodle.hu-berlin.de/) on a weekly basis. It is important to carefully read the exercise and submission instructions (e.g. concerning submission content, deadlines, file naming, and formats). Please contact us in case of doubt. 

Submission deadline for the weekly exercise is always the following monday at 10am. Please provide single file submissions in the original format, in case you have to submit multiple files, create a *.zip archive. Please use the naming convention indicating session number and family name of all students in the respective team, e.g. 's01_hammer_spengler_okujeni_rufin.pdf'.

Common disk space may be needed for group works. We suggest you use shared repositories in platforms like [HU-Box](https://www.cms.hu-berlin.de/de/dl/speicherdienste/HU-Box), or store your data in a shared folder on drive O:/ (access requires VPN connection).
...
* [Google Earth Pro](http://www.google.de/earth/download/gep/)

* [QGIS v3.1X.X](https://www.qgis.org/)
* [EnMAP Box QGIS Plugin](https://enmap-box.readthedocs.io/en/latest/usr_section/usr_installation.html)

* LibreOffice, Microsoft Office, or other software packages containing word processors spreadsheet tools
...
# Accuracy assessment
1. Introduction to Accuracy Assessment 
2. Exercise 

## 1. Introduction to Accuracy Assessment 
* Maps derived from remotely sensed data always contain classification errors. 

* The severity of these errors depends on many factors, i.e. 
  + input data (satellite images, training data)
  + classification algorythms (Random Forest vs. Maximum Likelihood) 
  + complexity and separability of classes (forest / non-forest vs. various urban surface materials)

![](docs/fig/classification_vs_reality.png)
  
* A quantitative assessment of the map accuracy and errors is necessary. 

* When validating a classification result, the classification (estimate) is compared to the validation data (ground truth) at random. 

* Conditions for that are: 
  + Validation data should be created inpendently from training data. 
  + Validation data should be sampled at random, sometimes this is not practicable. 
  + Validation and training data should not be autocorrelated, i.e. by defining a minimum distance between points.

![](docs/fig/validation_example.png)

## The Confusion Matrix 
* The **confusion matrix** is the basis of all accuracy analysis and compares validation data (ground truth) with the classified pixels of the map (estimate). 

![](docs/fig/validation_example2.png)

![](docs/fig/validation_example3.png)

* Class information obtained from the validation data appear in the columns of the matrix, pixels from the classification appear in the rows of the matrix. 

* According to that the values in the diagonal are the correctly classified pixels. 

![contrasting this example, usually only samples of validation data exist](docs/fig/validation_example4.png)


## The Overall Accuracy 
* A common error estimate is the **Overall Accuracy** which expresses the ration of correctly classified pixels in percent. 

![](docs/fig/overall_accuracy.png)

* Note: The OA does not provide information on which class was classified well or bad, nor if single classes were over- or underestimated. 

* Class specific error estimates are the **User's Accuracy** with the Error of Commission and the **Producer's Accuracy** with the  Error of Omission 

## User's Accuracy 
* The **User's Accuracy** is a *class specific* error estimate. It expresses (in %) the amount of correctly classified pixels divided by the amount of validation pixels. 

* The **Error of Commission** is derived from it. The error of commission expresses the percentage of falsly classified pixels when compared to validation data. 

![](docs/fig/classification_vs_reality.png)

* User´s Accuracy: 
Forest: 100% (6/6)  ->  100% of the area classified as forest is really forest. 
Cropland: 33% (1/3) ->  33% of the area classified as cropland is really cropland.

* Error of Commission:
Forest: 0% (0/6)    -> 0% of the area classified as forest was wrongly classified. 
Cropland: 67% (2/3) -> 67% of the area classified as cropland was wrongly classified. 

## Producer's Accuracy 
* The **Producer's Accuracy** is also a *class specific* error estimate. It expresses (in %) the amount of correctly classified pixels of a class divided by the amount of all validation pixels of that class. 

* The **Error of Omission** is derived from it. It expresses the percentage of pixels that were uncorrectly classified when compared to all validation points from that class.

![](docs/fig/classification_vs_reality.png)


* Producer´s Accuracy:
  * Forest: 75% (6/8)    -> 75% of the true forest was correctly classified.
  * Cropland: 100% (1/1) -> 100% of the true cropland was correctly classified. 


* Error of Omission:
  * Forest: 25% (2/8)    -> 25% of the true forest was forgotten. 
  * Cropland: 0% (0/1)   -> 0% of the true cropland was forgotten.

## Introduction to Accuracy Assessment 
* Class specific error estimates (User's and Producer's Accuracy) can easily be derived from the confusion matrix. 

* The class specific errors (Error of Commission and Omission) can easily be calculated:  

  *100 % - User's Accuracy = Error of Commission*   


  *100 % - Producer's Accuracy = Error of Omission* 

* Note: to 'omit' area leads to **underestimation**
* Note: to 'commit' errors leads to **overestimation**
    
 *OA = amount of correctly classified pixels / mount of validation pixels*
![](docs/fig/overall_accuracy2.png)
